...
function Synth() {
  ...
  const play = (freq, name = 'unnamed') => {
    ...
    if (oscillators.current.find((o) => o.name === name)) {
      oscillators.current.find((o) => o.name === name)
        .oscGain.gain.cancelScheduledValues(audioContext.current.currentTime);
    }
    ...
    if (oscillators.current.find((o) => o.name === name)) {
      ...
      oscillators.current.find((o) => o.name === name)
        .osc.disconnect();
      oscillators.current.find((o) => o.name === name)
        .osc.connect(oscillators.current.find((o) => o.name === name).oscGain);
      oscillators.current.find((o) => o.name === name)
        .isPressed = true;
      oscillators.current.find((o) => o.name === name)
        .timeStarted = audioContext.current.currentTime;
    } else {
      console.log('new osc');
      oscillators.current
        .push(createOscillatorGroup(freq, unisonWidth, name, audioContext.current));
    }
    // filter
    setupFilterNode(gainNode.current);
    setupReverbNode(audioContext.current, oscillators.current
      .find((o) => o.name === name).oscGain, filterNode.current, reverbNode.current);
    ...
    oscillators.current.find((o) => o.name === name)
      .oscGain.gain.linearRampToValueAtTime(gain, attackEndTime);
    oscillators.current.find((o) => o.name === name)
      .oscGain.gain.setTargetAtTime(sustain * gain, attackEndTime, decayDuration);
  };

  // TODO support decay
  const currentOscGainValue = (now, name) => {
    const currentGain = ((now - oscillators.current
      .find((o) => o.name === name).timeStarted) / (attack * STAGE_MAX_TIME)) * gain;
    if (!currentGain) {
  ...
  const stopOscillators = (name = 'unnamed') => {
    ...
    oscillators.current.find((o) => o.name === name)
      .oscGain.gain.setValueAtTime(currentOscGainValue(now, name), now);
    oscillators.current.find((o) => o.name === name)
    ...
    <div className="synth">
      <div className="settings">
        ...
        <Filter
          ...
          filterQ={filterQ}
          labelFilterFrequency={filterNode.current !== null && filterNode.current.frequency.value}
        />
        <EchoDelay
        ...
        />
        <Reverb
          setReverbWetRatio={setReverbWetRatio}
        ...
        <Keyboard
          ...
          oscillators={oscillators.current}
          refC={refC}
        />
      </div>
